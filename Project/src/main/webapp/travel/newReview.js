...
function ajaxBoardList() {
	...
	$.getJSON("rvlist.json", {
		...
		for ( var i in arr) {
			...
			console.log(b)
			contents += "<hr>" + 
			    "<div style='border:1px solid black;'>" + "<dl>"+ "<img src='img/2253263.jpg'  >" +
				"<dt >"+arr[i].reviewboardno+"</dt>"+
				"<dt><a class='titleLink' href='#' data-no='"+arr[i].reviewboardno+"'>"+
				  arr[i].title +" "+(arr[i].commentCount==0?"":"("+arr[i].commentCount+")")+"</a></dt>"+
				"<dt><a class='titleLink2' href='#' data-no2='"+arr[i].memberno+"'>"+arr[i].membername+"</a></dt>"+
				"<div  style='text-align: right;  width:400px;'>" + "<dt>"+arr[i].createdDate+"</dt>"+
